...
\item [WebRTC] is a protocol that allows real-time communication from browser
  ...
  network itself distributes the load of these connections establishement among
  the members.
\end{asparadesc}
...
peer sampling protocol providing 
\begin{inparaenum}[(i)]
\item a logarithmically growing partial view size compared to the network
  membership (like \SCAMP{}),
\item using neighbour-to-neighbour interactions to establish connections (like
  \CYCLON{}),
\item  quickly converging to a random network.
\end{inparaenum}
Such protocol would allow gossiping on top of WebRTC. Hence, it would open the
gate to a wide variety of distributed and decentralised applications requiring
a scalable broadcast.
