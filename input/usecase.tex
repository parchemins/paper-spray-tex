...
systems~\cite{Frey09Middleware,Zhang07Understanding,monod:THESIS} use
...
variant adopted
by~\cite{FlightPath,Frey09DSN,monod:THESIS,Zhang07Understanding}. Nodes
gossip advertisement messages for available packets to \emph{fanout}
other nodes, which then pull the packets they need.
Algorithm~\ref{algo:threephase} details the protocol's
operation. First, with a time interval of $\delta$ ($200ms$
...
achieve this in a large scale setting: (i) refreshing the view of the
peer-sampling protocol (e.g. Cyclon, Spray) before sending each
advertisement packet, or (ii) using a view that is much larger than
the gossip fanout and refreshing it less often. Due to the relatively
small size of view-exchange messages, solution (ii) achieves the best
trade-off, while solution (i) would be extremely impractical and waste
a significant amount of bandwidth just for packet
headers---\cite{Frey09DSN,Frey09Middleware} have each node send $5$
advertisement packets per second.

For this reason, \cite{frey:hal-01479885} follows solution (ii) and sets the
fanout to $\ln(N)+c$ (according to theory\cite{PRD}), $N$ being the
number of participating peers, and $c$ a positive constant.
...
\item [Description:] 
  ...
  protocol in~\cite{Frey09DSN,monod:THESIS,frey:hal-01479885} and model an
