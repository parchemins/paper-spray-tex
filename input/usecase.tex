...
\CRATE builds a network of browsers where each browser is able to communicate
...
state-of-the-art~\cite{tolgyeski2009adaptive,voulgaris2005cyclon}, \SPRAY allows
the diffusion cost to adapt to the network size without any probing mechanism or
any central authority.  The developers do not have to overestimate the
parameters of peer-sampling protocols to get a system that well behave for
largest networks as well as for small networks. This would increase the cost of
maintaining connections alive (e.g. in WebRTC context) and more importantly the
cost of protocols built on top of it (e.g. broadcasting protocols).  Without
intervention of the developers or the users, \SPRAY makes sure that small
networks do not pay the price of large networks.
