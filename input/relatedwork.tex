...
figure~\ref{fig:webrtc}. This drastically impacts \SCAMP`s failure
probability of establishing connections. Let $P_f$ be the probability that an
...
corresponds to the probability that each element (arc and peer) in the path
of size $k+1$ stays alive during their part of the dissemination (i.e.,
otherwise, they are allowed to crash or leave). In the context of WebRTC,
the offer ticket must travel back to its emitter. As consequence, the elements
