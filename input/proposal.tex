...
In \SPRAY's shuffling protocol, each of the involved peers sends half
of its partial views to the other. After integration, their view sizes
both tend to the average of their initial sizes their sum remains
unchanged. In order to keep the arc number invariant, the partial
views of \SPRAY are multisets. If a peer receives an already known
reference, it still stores it, yet as a duplicate. Thus, the \SPRAY's
shuffling protocol never increases nor decreases the arc count.
...
negligible as the network grows (cf. Figure~\ref{fig:duplicates}).
...
each of these peer. On average, one of these peers will likely remove
...
when a connection establishement fails. \SPRAY calls the
