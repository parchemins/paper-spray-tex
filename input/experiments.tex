...
\item[Description:] The average local clustering
  ...
  Out of fairness, we configure the representative of fixed-size approaches
  \CYCLON to provide $7$ neighbors which is close to the number of neighbors
  provided by \SPRAY when the network size reaches 1k peers
  ($\ln(1000)\approx 7$). Compared to \SPRAY, \CYCLON is oversized for 0.1k
  peers and undersized for 10k peers and 100k peers. During exchanges, the peers
  using \CYCLON shuffle $3$ out of their $7$ neighbors.
...
\item [Description:] To evaluate the accuracy of this network size estimator, we
  ...
  $|P^t_x| \approx \ln(|V^t|)$. To estimate the network size, a Peer $x$ processes:
  \begin{equation*}
...
\item [Reasons:] Particularly in the case of local only knowledge, the standard
  ...
  some peers have 12 neighbors in their partial view. Since Peer $x$'s
  estimation using local only knowledge is
  $\smash{\widehat{V^t_x} = \exp(|P^t_x|)}$, increasing by 1 the view size
  results in a much larger estimate. Hence the sudden increase in standard
  deviation at such network sizes. The standard deviation decreases as the
  actual network size approaches the overestimated size.
