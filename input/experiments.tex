...
\begin{asparadesc}
\item[Objective:] To show the impact of adaptivity when the network size
  changes over time.
...
\begin{figure}
  ...
  \caption{\label{fig:degeneration}\SPRAY and \SCAMP in network subject to
    failures in the connection establishement. }
\end{figure}

\begin{asparadesc}
\item[Objective:] To show that the degeneration of \SCAMP in presence of
  connection establishement failures quickly endangers the connectedness of the
  network.
  % \SPRAY{}, especially when using \emph{handshaking}, is
...
\item[Description:] We measure both the arcs count and the number of weak
  components in the network. The simulations involve \SPRAY and \SCAMP running
  over 70k cycles. The network intially contains 10k members. The probability
  that a message fails to arrive to its destination (each hop) is set to
  $10^-3$. To establish a connection, we use the WebRTC three-way handshake,
  i.e., the initial peer emits an offer ticket, the arrival peer stamps the
  ticket, the initial peer finalizes the connection using the stamped ticket
  (cf. Section~\ref{sec:relatedwork}).
  % As previously shown, the number of arcs in the
  % network should be $|\mathcal{N}|\log{|\mathcal{N}|}$ for the overlay to be
  ...
  % network
  % is disconnected.  Unlike with strong component where the direction is taken
  % into account, \SPRAY{} cannot repair itself when more than one weak
  % component
  % is present.  The experiment involves handshaking and is conducted with an
  % overlay consisting of $10,000$ peers over a period of $100,000$ cycles and
  % a
  % probability of $\frac{1}{1000}$ that a link fails. \TODO{Moar on calcs}
