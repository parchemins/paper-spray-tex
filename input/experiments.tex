...
In this section, we evaluate the benefits that \SPRAY can bring to protocols
built on top of it. Using the example of a broadcast mechanism, we expect \SPRAY
to keep a stable delivery rate on messages supporting network size
variations. Then, we evaluate how the adaptiveness of \SPRAY impacts common
metrics of peer sampling performance including clustering coefficient, average
shortest path length, in-degree distribution, and arc count. We compare \SPRAY
with a representative of fixed-size partial view approaches, namely \CYCLON. We
expect \SPRAY and \CYCLON to exhibit similar behaviors when \CYCLON is optimally
configured in advance to handle the network size. We expect \SPRAY to save
resources when \CYCLON is oversized. We expect \SPRAY to be more robust when
\CYCLON is undersized. Finally, we expect \SPRAY to keep a negligible number of
duplicates in its partial views. The experiments run on the \PEERSIM
...
\item [Description:] Broadcast protocols extensively use the partial views
  ...
  partial view to whom it sends the message. Each peer receiving such message
  broadcasts it once, in an identical way. The message transitively reach all
  members. The number of chosen neighbors is called the \emph{fanout}. Since
  broadcasts happen more often than shufflings of sampling protocols, the
  partial view size is inflated and the fanout is significantly smaller than it.
  Thus, even between two shuffles, the peer-sampling can provide different peers
  at random. In this experiment, we configure the size of \CYCLON's partial view
  to 30. Its fanout roughly targets 100 peers ($\ln(100)\approx 5$). To provide
  high reliablity of messages propagation at 100 peers, we set the fanout to
  $\ln(100)+1$, then $\ln(100)+3$. We configure \SPRAY to have partial views
  size equal to $6.\ln(|V^t|)$ -- instead of adding only 1 arc targeting its
  contact (cf. Section \ref{subsec:joining}), the newcomer adds 6 arcs. The
  fanout is set to 1 sixth of the view size (i.e. $\ln(|V^t|)$). Similarly to
  \CYCLON, a small value ($1$ then $3$) is added to this fanout, for the sake of
  reliability. The measurements concern the full delivery rate of messages over
  1k messages, i.e., the messages that reach all members of the network. The
  network size grows up till 2000 peers.
\item [Results:] Figure~\ref{fig:hardrate} shows the result of this
  simulation. First, we observe that the rate of messages delivery using \CYCLON
  suffers from an endless decreasing. The highest fanout leads to better
  results. On the opposite, \SPRAY remains stables, despite the presence of
  small jumps. A fanout set to $\ln(|V^t|)+1$ gives a full delivery rate above
  90 percent. A fanout set to $\ln(|V^t|)+3$ is very close of 100
  percent. Overall, the broadcast mechanism benefits from the adaptiveness of
  \SPRAY's partial view which allows it to scale regarding the network size.
\item [Reasons:] From~\cite{erdos1959random}, we know that the sharp
  threshold of connectedness of random graphs is $\Theta(\ln(|V|))$. Regarding
  the broadcast, it means that, with a sufficiently random and large partial
  view, a fanout set accordingly provides around 50 percent of full delivery
  rate. At this point, incrementing the fanout improves drastically the
  resulting rate, but the contribution of each increment diminishes compared to
  the previous increment. The broadcast protocol built on top of \CYCLON has a
  constant fanout size set for a specific network size. As long as the network
  is smaller than this value, the full delivery rate stays high. However, this
  quickly decreases at higher network size. Using \SPRAY, the fanout growth
  follows the growth of the partial view size which scales logarithmically
  compared to the network size. Hence, network size variations impact the
  fanout. The full delivery rate provided by the broadcast mechanism remains
  stable. Since the values manipulated by peers -- partial view size and fanout
  -- are integers, the measurements make small jumps when a rounded value
...
\item [Objective:] To show how the full delivery rate behaves during a buzz,
  ...
  departures.
\item [Description:] The configurations of peer-sampling protocols are similar
  to the previous experiment. A broadcast built on top of \CYCLON with two
  setups: partial view size of 30 with a fanout set to $\ln(100)+1$ and a fanout
  set to $\ln(100)+3$. A broadcast built on top of \SPRAY with a partial view
  growing as $6.\ln(|V^t|)$ and fanout as $\ln(|V^t|)+1$ and $\ln(|V^t|)+3$. The
  simulation starts with 100 peers. The network quickly reaches 10k peers during
  the buzz. Then the network size decreases to 3k members. In this experiment,
  we measure the full delivery rate of messages over 100 messages per
  measurement.
\item [Results:] Figure~\ref{fig:peak} shows the results of this experiment. The
  top part of the figure shows the evolution of the network size over time. The
  bottom figure shows the full delivery rate over time. Similarly to the
  previous experiment, Figure~\ref{fig:peak} shows that using \CYCLON and a
  predefined fanout works well until the network size reaches unexpected
  dimensions. During the peak, the delivery rate falls quickly. On the opposite,
  the broadcast mechanism built on top of \SPRAY adapts the fanout to the
  network size automatically. Thus, it does not suffer for long of massive
  joinings. Yet, we observe a decreasing delivery rate during the departures of
  peers, in particular when the fanout is configured to $\ln(|V|)+1$. Overall,
  the broadcast mechanisms involving \SPRAY are more resilient to quick membership
  changes.
\item [Reasons:] The fanout of configurations involving \CYCLON is
  constant. When the number of peer in the network exceeds the expectations, the
  delivery rate quickly degenerates. Concerning \SPRAY, the fanout follows the
  evolution of the network thanks to adaptive partial views. Yet, \SPRAY detects
  the departures during the shuffling phase. Thus, the broadcast may use stale
  arcs from partial views which negatively impacts the delivery rate. Since both
  \CYCLON and \SPRAY clean their partial views over time, the delivery rate
  recovers its expected value.
\end{asparadesc}
