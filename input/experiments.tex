...
\begin{asparadesc} \item[Objective:] \SCAMPLON{}, especially when using
        ...
        we compare instead with another adaptive probabilistic protocol,
        \SCAMP{}.
        This experiment focuses on two features: the total number of arcs in the 
        graph and number of weak components.
\item[Description:]
    ...
    component of a graph is a maximum subgraph where the directions are ignored.
    Naturally, we want the number of weak components to be $1$, meaning that the
    whole network is connected. When the overlay consists of more than one
    component, the network is disconnected.

\item[Results:] \item[Reasons:] 
