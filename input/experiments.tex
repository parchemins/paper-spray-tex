...
\item[Description:] Counting the strong components in a network allows
  ...
  neighbors which targets 8k peers. The network contains 10k members. We
...
\item[Reasons:] The random peer sampling approaches \CYCLON and \SPRAY expose
  ...
  affect the whole network. The direction of arcs is more important to the
  information dissemination than it is to the peer-sampling protocol. Indeed, if
  an arc constitutes the last link between two clusters of the network, the
  messages from one of these cluster cannot reach the other one. Yet, this arc
  is enough for the peer to start the shufflings which will eventually populate
  their views with members from each other's cluster. Hence, the network repairs
  itself.  When clusters become fully disjoints, neither \CYCLON nor \SPRAY is
  able to repair the network.  % A smaller growth could be obtained because the
