...
count, connected components. We use \CYCLON as baseline with a
fixed-size view for experiments relative to adaptivity. We expect
similar behavior when network size is optimal for \CYCLON. We expect
\SPRAY to save resources when \CYCLON is oversized. We expect \SPRAY
to be more robust when \CYCLON is undersized. Finally, we expect
\SPRAY to keep number of duplicates in partial view very low. We use
\SCAMP as baseline for experiments relative to connection failures. We
expect \SPRAY to tolerate connection failures. We do not use \SCAMP
for experiments relative to adaptivity as SCAMP fails to maintain a
random graph in WebRTC context.
...
\begin{figure}
  ...
  \caption{\label{fig:degeneration}\SPRAY and \SCAMP in network subject to
    failures in the connection establishments. The x-axis denotes the elapsed
    time in cycles ($10^3$-scaled). The y-axis of the top figure denotes the
    global number of arcs ($10^3$-scaled). The y-axis of the bottom figure
    denotes the ratio of weak components over the current network size.}
\end{figure}

\begin{asparadesc}
\item[Objective:] To show that the degeneration of \SCAMP in presence of
  failures in the connection establishments quickly endangers the connectedness
  of the network.
\item[Description:] We measure both the arcs count and the number of weak
  components in the network. The simulations involve \SPRAY and
  \SCAMP\footnote{A modified version of \SCAMP whose periodic protocol works
    properly when there is no connection failures. Available at
    \url{https://github.com/justayak/peersim-spray}} running over 70k
  cycles. The network initially contains 10k members. The probability that a
  message fails to arrive to its destination (each hop) is set to $10^-3$. To
...
\item[Results:] Figure~\ref{fig:degeneration} is comprised of two parts. The
  top figure shows the arc count of both random peer samplings while the bottom
  figure shows the weak components of the network. Both \SPRAY and \SCAMP
  degenerate over time. The top figure shows that they both starts
  approximately at the same values. However, the \SCAMP's arc count decreases
  very rapidly. The bottom figure depicting the weak components (i.e. the
  number of fully disjoint part) shows that parts of \SCAMP network directly
  disconnect from each other. On the other hand, \SPRAY becomes disconnected at
  the very end only, when the number of connections is not sufficient to ensure
  any connectedness.
\item[Reasons:] Each connection establishment of \SCAMP relies on a random
  walk.  Each hop of this random walk is an opportunity of failure. Since there
  is no routing in such network, the only way for a stamped ticket to come back
  to its emitter is the path it traveled in the first place. Hence, all peers
  belonging to the path must stay alive until the stamped ticket comes back in
  order to consistently forward it. Thus, the probability to fail to establish
  a connection depends of the length of the random walk which is generally much
  higher than a neighbor-to-neigbhor connection establishments (as in \SPRAY
  or \CYCLON). Hence the quick degeneration of the \SCAMP arc
  count. Furthermore the network using \SCAMP becomes quickly partitioned
  \begin{inparaenum}
  \item because its arc count decreases and
  \item because its periodic protocol is not cautious.
  \end{inparaenum}
  Indeed, it immediately cuts the incoming arcs of the involved peer and
  assumes that the new connections spread in the network will establish.  When
  it does not, the peer becomes disconnected. When its neighbors execute the
  periodic protocol, they delete their reference in its partial view. The peer
  becomes disconnected. The second observation concerns \SPRAY: it degenerates
  too. Indeed, \SPRAY does not differentiate a crashed peer from a dead arc. It
  always applies the probabilistic removal described in
  Section~\ref{subsec:leaving}. It worth noting that in the WebRTC context, we
  can distinguish these two cases. Therefore, it does not constitute an issue.
\end{asparadesc}
