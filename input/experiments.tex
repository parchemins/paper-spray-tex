...
\item[Description:] The average local clustering
  ...
  coefficient, the more peers are tied together. Random graphs have a
  coefficient close to 0. The runs concern 0.1k, 1k, 10k and 100k peers. The
  representative of the fixed-size approach is \CYCLON which is optimally
  configured for 1k peers: $\ln(1000)\approx 7$ neighbors. During exchanges, the
  peers using \CYCLON shuffle $3$ out of their $7$ neighbors. \CYCLON is
  oversized for 0.1k peers and undersized for 10k peers and 100k peers.
...
%    priori}, they constitute a constraint to the convergence speed.  
  ...
  the same average partial view size (\SPRAY 7.1 vs \CYCLON 7), hence identical
  clustering coefficient measurements. By extending the reasoning, this also
  explains why \SPRAY yields lower $\overline{C}$ values when \CYCLON is
  oversized, and why it yields higher values when \CYCLON is undersized.
\end{asparadesc}
