...
for \CYCLON. We expect that \SPRAY saves resources when \CYCLON is oversized. 
Also, \SPRAY should be more robust when \CYCLON is undersized. Finally, we expect
\SPRAY to keep the number of duplicates in partial view very low. We use \SCAMP as
...
\item[Reasons:] \CYCLON starts with a lower clustering coefficient because each
  ...
  (\SPRAY 7.4 vs \CYCLON 7). By extending the reasoning, it explains why
...
\item[Reasons:] The arc count of both \CYCLON and \SPRAY remains constant over
  ...
  connections spread in the network will establish.  Since they do not, the
  peers eventually become disconnected. Also, when neighbors execute the
  periodic protocol, they delete their reference in their partial view. In such
  case, peers become disconnected and partitions quickly appears.
\end{asparadesc}
