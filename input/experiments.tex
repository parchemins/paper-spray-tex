...
\item[Reasons:] \CYCLON starts with a lower clustering coefficient because each
  ...
  lower clustering coefficient of \SPRAY on this run (\SPRAY 7.4 vs \CYCLON
  7). By extending the reasoning, this also explains why \SPRAY yields lower
...
% None of existing algorithms
% provide satisfying complexities in both communication and space.  In this paper,
% we propose 
...
% We evaluate \CRATE on large-scale experiments in the Grid'5000 testbed involving
% uptill 600 participants. As expected, we observe a logarithmic progression of
% the generated traffic according to the size of the shared document and the
% number of participants.

\begin{figure*}
  \centering
  \subfloat[Figure A][\label{fig:partialview}Average partial view size.]{
    \includegraphics[width=0.48\textwidth]{img/partialview.eps}}
  \hspace{10pt}
  \subfloat[Figure B][\label{fig:traffic} Average traffic per second.]{
...
\item [Results:] Figure~\ref{fig:partialview} shows the average partial view
  ...
  generated by editors during the experiment.  \TODO{As figure~\ref{fig:churn}
    suggests, \CYCLON which is generally oversized to accommodate with any
    network size, would have an identical traffic for any of the experiments.}
\item [Reasons:]
\end{asparadesc}
