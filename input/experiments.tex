...
components. We use \CYCLON as baseline with a fixed-size view. We expect
similar behavior when network size is optimal for \CYCLON. We expect \SPRAY to
save ressources when \CYCLON is oversized. We expect \SPRAY to be more robust
when \CYCLON is undersized. Finally, we expect \SPRAY to keep number of
duplicates in partial view very low.
...
\item[Description:] We measure both the arcs count and the number of weak
  ...
  \SCAMP\footnote{A modified version of \SCAMP whose periodic protocol works
    properly when there is no connection failures.} running over 70k
  cycles. The network intially contains 10k members. The probability that a
...
\item[Results:] Figure~\ref{fig:degeneration} is comprised of two parts. The
  ...
  approximatively at the same values. However, the \SCAMP's arc count decreases
  ...
  any connectedness.
\item[Reasons:] Each connection establishement of \SCAMP relies on a random
  ...
  to its emitter is the path it travelled in the first place. Hence, all peers
  ...
  higher than a neighbor-to-neigbhor connection establishements (as in \SPRAY
  or \CYCLON). Hence the quick degeneration of the \SCAMP arc
  count. Furthermore the network using \SCAMP becomes quickly partionned
  \begin{inparaenum}
  \item because its arc count decreases and
  \item because its periodic protocol is not cautious.
  \end{inparaenum}
  Indeed, it immediatly cuts the incomming arcs of the involved peer and
  ...
  too. Indeed, \SPRAY does not differenciate a crashed peer from a dead arc. It
