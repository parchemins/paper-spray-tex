...
\begin{algorithmic}[1]
  \Statex
  \initially
  \State $B$ \hfill \comm{buffer of messages}
  \State $b$ \hfill \comm{number of advertised messages}
  ...
  \State \textbf{let} $advertisement \leftarrow getIdentifiers(B,\,b)$; \hfill \comm{$b$ distinct random message Ids}
  ...
  \State \textbf{let} $message \leftarrow getMessage(B,\,id)$; \hfill \comm{get the requested message from the buffer}
  \State $sendTo(o, \, message)$; \hfill \comm{finally send the message}
  \EndFunction
